...
| **constructibility** | Evidence describing whether CodeAtlas can materialize one callable receiver, parameter, or result directly, only through a declared factory, not at all, or not yet known. | `CallableContract`; later language fuzz adapter consumes it | A type annotation alone does not prove constructibility. Unknown never defaults to direct. |
