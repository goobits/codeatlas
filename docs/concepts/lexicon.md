...
| **code harness** | External-only runtime files and one native-engine command generated from an exact callable contract and bound into a code-fuzz plan. | Language adapter generates; `fuzz::code` identities; `execution` runs | A harness translates evidence into native values. It does not parse source, own limits, grant effects, or create source-local projects. |
| **fuzz marker** | The exact planned `CODEATLAS_FUZZ=1` context supplied to a sandboxed callable harness. | Code-fuzz workload protocol | It is audit/context evidence, never authorization or isolation. A target branch on the marker makes the run `alternate_behavior`. No source directive can enable it. |
