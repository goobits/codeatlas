...
pub(super) fn remove_nested_workspace_contexts(
    ...
    for project in projects {
        ...
        let discovered_sources = discovery
            ...
            .filter_map(|source| {
                let relative =
                    codeatlas_source::paths::normalize_relative_path(source, &project.root);
                codeatlas_source::source_policy::source_argument(&relative)
                    .is_some()
            ...
            let matched = discovered_sources
                ...
                .filter(|(source, relative)| {
                    matchers.iter().any(|matcher| matcher.is_match(relative))
                        && codeatlas_source::source_discovery::is_visible_with_patterns(
                            &project.root,
                            source,
            ...
            let all_patterns_are_nested = matched.is_empty()
                && context.entrypoints.iter().all(|pattern| {
                    ...
                    project.excluded_roots.iter().any(|excluded| {
                        let relative = codeatlas_source::paths::normalize_relative_path(
                            excluded,
                            &project.root,
                        );
