...
pub(crate) fn render(report: &ScanReport) -> String {
    ...
    if issue_count == 0 {
        ...
        output.push_str(&format!("  {} public symbols across {} files.\n",
            report.symbols.len(),
    ...
    for (i, unused) in report.unused_public.iter().enumerate() {
        ...
        output.push_str(&format!("   {} This is exported but nothing imports it.\n", "→".dimmed()));
    ...
    output.push_str(&format!("  {} Review each unused export above\n", "1.".dimmed()));
    output.push_str(&format!("  {} Either remove the export or make it internal\n", "2.".dimmed()));
    output.push_str(&format!("  {} Run {} to verify fixes\n\n", "3.".dimmed(), "codeatlas ci".cyan()));

    output.push_str(&format!("{}\n", "Quick commands:".white().bold()));
    output.push_str(&format!("  {}  See full public API\n", "codeatlas scan".cyan()));
    output.push_str(&format!("  {}   Generate architecture diagram\n", "codeatlas map".cyan()));
    ...
    match language {
        Language::Rust => {
            if is_function {
                ...
                "Change `pub trait` to `pub(crate) trait` or keep if intentionally public".to_string()
            } else {
                "Change `pub` to `pub(crate)` or remove the export".to_string()
            }
        }
        Language::TypeScript => {
            "Remove the `export` keyword or delete if unused".to_string()
        }
        Language::Python => {
            "Prefix with underscore (e.g., `_function_name`) or remove".to_string()
        }
        Language::Unknown => {
            "Remove the public export or make it internal".to_string()
        }
    }
}
