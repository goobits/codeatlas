...
use super::digest::{digest_bytes, DigestKind, TypedDigest};
...
pub(crate) fn parse(
    source: &[u8],
    limits: ParseLimits,
) -> Result<ParsedDocument, ArchitectureError> {
    ...
    let yaml: YamlValue = serde_yaml::from_slice(source).map_err(|error| {
        ...
        ArchitectureError::new(code, message)
    })?;
    Ok(ParsedDocument {
        value: convert_value(&yaml, 0, limits)?,
        source_document_digest: digest_bytes(DigestKind::SourceDocument, source),
    })
}

fn scan_prohibited_syntax(text: &str) -> Result<(), ArchitectureError> {
...
fn scan_line_tokens(line: &str, line_number: usize) -> Result<(), ArchitectureError> {
...
fn prohibited(code: &str, feature: &str, line: usize) -> ArchitectureError {
    ArchitectureError::new(code, format!("{feature} are prohibited at line {line}"))
}
...
fn convert_value(
    ...
    depth: usize,
    limits: ParseLimits,
) -> Result<Value, ArchitectureError> {
    ...
    match value {
        ...
        YamlValue::Tagged(_) => Err(ArchitectureError::new(
            "yaml.tag-prohibited",
...
mod tests {
    ...
    #[test]
    fn rejects_prohibited_yaml_features_for_their_exact_reason() {
        let cases = [
            ("a: 1\na: 2\n", "yaml.duplicate-key"),
        ...
        for (source, code) in cases {
            let error = parse(source.as_bytes(), ParseLimits::default()).expect_err("invalid YAML");
            assert_eq!(error.diagnostic.code, code, "{source}");
        }
    }

    #[test]
    fn source_digest_preserves_exact_bytes() {
    ...
    #[test]
    fn quoted_yaml_metacharacters_remain_literal() {
        let document = parse(
            b"value: \"!literal &value *other ${TOKEN}\"\n",
            ParseLimits::default(),
        )
        .expect("literal");
        assert_eq!(document.value["value"], "!literal &value *other ${TOKEN}");
    }

    #[test]
    fn enforces_size_depth_and_string_limits() {
        let size_error = parse(
            b"long: value\n",
            ParseLimits {
        ...
        .expect_err("string");
        assert_eq!(string_error.diagnostic.code, "resource.string-too-large");
    }
}
