...
pub fn parse_module_info(file_path: &Path, root_dir: &Path) -> Result<TypeScriptModuleInfo> {
...
pub fn parse_source(source: &str, relative_path: &str) -> Result<TypeScriptModuleInfo> {
...
pub fn parse_syntax_tree(file_path: &Path) -> Result<(Module, Lrc<SourceMap>)> {
