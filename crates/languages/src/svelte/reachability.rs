use super::parser;
use crate::typescript::parser::TypeScriptModuleInfo;
use anyhow::Result;

pub fn parse_source(relative_path: &str, source: &str) -> Result<TypeScriptModuleInfo> {
    let mut combined = crate::typescript::parser::parse_source("", relative_path)?;
    for script in parser::script_blocks(source) {
        let mut info = crate::typescript::parser::parse_source(&script.source, relative_path)?;
...
<p>{localValue}</p>"#;
        ...
        assert_eq!(
            ...
            crate::typescript::parser::DynamicDependencyTarget::Literal(
