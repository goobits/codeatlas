...
pub(super) fn collect_project_modules(
    ...
    let module_patterns =
        codeatlas_source::source_discovery::project_source_patterns(project, &discovery_patterns);
    let html_patterns = codeatlas_source::source_discovery::project_source_patterns(
        project,
    ...
    let discovery = codeatlas_source::source_discovery::discover_project_sources_with_patterns(
        project,
        &combined_patterns,
    );
    ...
    for source_path in discovery.files {
        ...
        let info = index.parse_file(
            ...
            |source| match language {
                SourceLanguage::Svelte => crate::svelte::reachability::parse_source(&path, source),
