...
impl DigestKind {
    pub const ALL: [Self; 10] = [
        Self::SourceDocument,
        Self::CanonicalModule,
        Self::ImportClosure,
        Self::ArchitectureClosure,
        Self::GoverningGraph,
        Self::PolicyClosure,
        Self::ReviewGraph,
        Self::ObservationContent,
        Self::ObservationEnvelope,
    ...
    fn domain_separator(self) -> String {
        format!("atlas.codeatlas.dev/{}/v0.1\n", self.name())
    }
}

#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd, Serialize, Deserialize)]
#[serde(transparent)]
pub struct TypedDigest(String);
...
pub fn digest_bytes(kind: DigestKind, bytes: &[u8]) -> TypedDigest {
    let mut hasher = Sha256::new();
    hasher.update(kind.domain_separator().as_bytes());
    hasher.update(bytes);
    TypedDigest(format!("sha256:{:x}", hasher.finalize()))
}

pub fn digest_value(kind: DigestKind, value: &Value) -> Result<TypedDigest, ValidationError> {
...
fn normalize_value(value: &Value) -> Result<Value, ValidationError> {
    match value {
...
#[cfg(test)]
mod tests {
    use super::{canonical_json_bytes, digest_bytes, digest_value, DigestKind, TypedDigest};
    use serde_json::json;
    use std::str::FromStr;
    ...
    #[test]
    fn canonicalization_rejects_floating_point_numbers() {
        let error = canonical_json_bytes(&json!({"confidence": 0.9}))
            .expect_err("floating point must fail");

        assert_eq!(error.diagnostic.code, "canonicalization.non-integer-number");
    }

    #[test]
    fn every_digest_family_has_a_distinct_domain() {
        let digests = DigestKind::ALL
            .map(|kind| digest_bytes(kind, b"same payload"))
            .into_iter()
            .collect::<std::collections::BTreeSet<_>>();

        assert_eq!(digests.len(), DigestKind::ALL.len());
    }

    #[test]
    fn semantic_digest_ignores_object_key_order() {
        let first = digest_value(
            DigestKind::CanonicalModule,
            &serde_json::from_str(r#"{"b":2,"a":1}"#).expect("first JSON"),
        )
        .expect("first digest");
        let second = digest_value(
            DigestKind::CanonicalModule,
            &serde_json::from_str(r#"{"a":1,"b":2}"#).expect("second JSON"),
        )
